...
register_metrics() ->
    [emqx_metrics:new(MetricName) || MetricName <- ['rabbitmq.client_connected',
                                                    'rabbitmq.client_disconnected',
                                                    'rabbitmq.client_subscribe',
                                                    'rabbitmq.client_unsubscribe',
                                                    'rabbitmq.session_created',
                                                    'rabbitmq.session_subscribed',
                                                    'rabbitmq.session_unsubscribed',
                                                    'rabbitmq.session_terminated',
                                                    'rabbitmq.message_publish',
                                                    'rabbitmq.message_deliver',
                                                    'rabbitmq.message_acked']].
...
on_client_connected(#{client_id := ClientId, username := Username}, 0, ConnInfo, _Env) ->
    emqx_metrics:inc('rabbitmq.client_connected'),
...
on_client_disconnected(#{client_id := ClientId, username := Username}, Reason, _Env)
    when is_atom(Reason) ->
    emqx_metrics:inc('rabbitmq.client_disconnected'),
...
on_client_subscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('rabbitmq.client_subscribe'),
...
on_client_unsubscribe(#{client_id := ClientId, username := Username}, TopicTable, {Filter}) ->
    lists:foreach(fun({Topic, Opts}) ->
      with_filter(
        fun() ->
          emqx_metrics:inc('rabbitmq.client_unsubscribe'),
...
on_session_created(#{client_id := ClientId}, SessInfo, _Env) ->
    emqx_metrics:inc('rabbitmq.session_created'),
...
on_session_subscribed(#{client_id := ClientId}, Topic, Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('rabbitmq.session_subscribed'),
...
on_session_unsubscribed(#{client_id := ClientId}, Topic, _Opts, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('rabbitmq.session_unsubscribed'),
...
on_session_terminated(#{client_id := ClientId}, Reason, _Env) when is_atom(Reason) ->
    emqx_metrics:inc('rabbitmq.session_terminated'),
...
on_message_publish(Message = #message{topic = Topic, flags = Flags}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('rabbitmq.message_publish'),
...
on_message_deliver(#{client_id := ClientId, username := Username}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
  with_filter(
    fun() ->
      emqx_metrics:inc('rabbitmq.message_deliver'),
      {FromClientId, FromUsername} = format_from(Message),
...
on_message_acked(#{client_id := ClientId}, Message = #message{topic = Topic, flags = #{retain := Retain}}, {Filter}) ->
    with_filter(
      fun() ->
        emqx_metrics:inc('rabbitmq.message_acked'),
        {FromClientId, FromUsername} = format_from(Message),
